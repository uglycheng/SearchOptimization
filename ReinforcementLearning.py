...
class DiscreteRL:
    ...
    def TD_policy(self,policy,gamma,max_iter=100):
        s_U = {i:0 for i in self.env.action}
        s_U[self.env.target] = self.reward[self.env.target]
    ...
    def Q_learning(self,gamma,eps,max_iter=100):
        ...
        for s,a_set in self.env.action.items():
            for a in a_set:
                s_Q[(s,a)] = 0
        s_Q[self.env.target] = self.reward[self.env.target]
        s_N = {i:0 for i in self.env.action}
        i = 0
        while i<=max_iter:
...
#                 print(s)
                ...
                s_next = self.env.cur_state
                s_N[s_next] += 1
                if s_next == self.env.target:
                    max_Q_next = s_Q[self.env.target]
                else:
